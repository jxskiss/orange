...
_M.help = [[
Usage: orange stop [OPTIONS]

Stop Orange with configurations(prefix/orange_conf/ngx_conf).
...
Examples:
 orange stop  #use `/usr/local/orange` as workspace with `/usr/local/orange/conf/orange.conf` & `/usr/local/orange/conf/nginx.conf`
 orange stop --prefix=/opt/orange  #use the `prefix` as workspace with ${prefix}/conf/orange.conf & ${prefix}/conf/nginx.conf
 orange stop --orange_conf=/opt/orange/conf/orange.conf --prefix=/opt/orange --ngx_conf=/opt/orange/conf/nginx.conf
 orange stop -h  #just show help tips
]]
...
function _M.execute(origin_args)
    ...
    xpcall(function()
        ...
        if result == 0 then
            logger:success("Orange stoped.")
        end
    end, function(e)
        logger:error("Could not stop Orange, error: %s", e)
        err = e
    end)
